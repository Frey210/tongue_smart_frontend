...
function Monitoring({ device, accessToken, role, onUnauthorized }: { device: Device; accessToken: string; role: Role; onUnauthorized: () => void }) {
  ...
  return <>
    ...
    {error && <p className="form-error" role="alert">{error}</p>}
    {selected && <><div className="measurement-tabs" role="tablist" aria-label="Jenis pengukuran">{selected.modules.map((module) => <button key={module} role="tab" aria-selected={tab === module} onClick={() => setTab(module as typeof tab)}>{module === "emg" ? "Aktivitas EMG" : module === "tongue_pressure" ? "Tekanan Lidah" : "Gaya Bibir"}</button>)}</div>
    ...
    <section className="live-monitor-grid"><article className="panel live-chart-panel"><div className="panel-title"><div><p className="eyebrow">{tabLabel}</p><h3>{tab === "emg" ? `Sensor EMG · elektroda ${selected.electrode_site?.replaceAll("_", " ") ?? "belum ditentukan"}` : tab === "tongue_pressure" ? `FSR lidah · titik ${fsrPoint.replaceAll("_", " ")}` : "Load cell HX711 · traksi"}</h3></div><span className="unit live-unit"><i />LIVE · {unit}</span></div><MedicalSignalChart points={results?.points ?? []} label={tabLabel} unit={unit} tone={tab === "emg" ? "navy" : tab === "tongue_pressure" ? "teal" : "blue"} emptyMessage="Pilih tahap lalu tekan mulai pengukuran. Simulator/device akan mengikuti kontrol ini."/><div className="measurement-controls">{tab === "tongue_pressure" && <select aria-label="Titik ukur FSR" value={fsrPoint} onChange={(event) => setFsrPoint(event.target.value)}><option value="median_anterior">Median anterior</option><option value="median_middle">Median tengah</option><option value="median_posterior">Median posterior</option><option value="lateral_left">Lateral kiri</option><option value="lateral_right">Lateral kanan</option></select>}{tab === "tongue_pressure" && <button className="secondary" disabled={selected.status !== "active" || saving} onClick={() => control("baseline")}>Mulai baseline</button>}<button className="primary" disabled={selected.status !== "active" || saving} onClick={() => control("recording")}>Mulai ukur {tab === "emg" ? "EMG" : tab === "tongue_pressure" ? "titik ini" : "gaya bibir"}</button><button className="secondary" disabled={selected.status !== "active" || saving || !controlState} onClick={() => control("paused")}>Jeda</button><button className="secondary" disabled={selected.status !== "active" || saving || !controlState} onClick={() => control("completed")}>Selesai tahap</button></div><p className="chart-summary">Live setiap 500 ms · {results?.batch_count ?? 0} batch · kualitas baik {results?.summary.quality.good ?? 0}</p></article>
